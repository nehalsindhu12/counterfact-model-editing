...
sys.path.append('/data/nehals/code/editing/unified-model-editing')
...
def main(
    ...
    if type(model_name) is str:
        ...
        local_model_path = "/data/akshat/models/Llama-2-7b-hf"

        model = AutoModelForCausalLM.from_pretrained(local_model_path, local_files_only=True).cuda()
        original_model = AutoModelForCausalLM.from_pretrained(local_model_path, local_files_only=True)
    ...
    if ds_name in 'cf':
        indices_filename = '/data/nehals/code/editing/orthogonal_fact_selection_cf_format/most_orthogonal.json'
    ...
    # Iterate through dataset
    for r, e in enumerate(range(0, len(sampled_indices), num_edits)):
        record_chunks = []
